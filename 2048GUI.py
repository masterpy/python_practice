...
# python 实现带图形界面的2048
# colors是定义2，4，8，不同分数的块颜色不同。
# 三个数值分别是RedGreenBlue。saveScore和loadScore是记录你的最高分，记录到文本文件里。
...
class Frame(wx.Frame):
    ...
    # 这里初始化各种字体，curScore为当前分数。
    # bstScore为最高分数。data为4×4的棋盘。
    # 然后while循环是在4×4棋盘上随机位置产生2个块2或4
    def initGame(self):
    ...
    # 所以界面的关键在drawAll，用户的动作响应在onKeyDown
    ...
    def putTile(self):
        ...
        if available:
            ...
            # 0.9的概率出2
            self.data[row][col] = 2 if random.random() < 0.9 else 4
            return True
        return False
    ...
    # 当用户按上或下方向时，slideUpDown对棋盘的列进行处理，当用户按左或右方向时，
    # slideLeftRight对棋盘的行进行处理
